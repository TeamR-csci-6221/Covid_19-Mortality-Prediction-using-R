...
server <- function(input, output) {
  ...
  observeEvent(input$submit_2, {
    ...
    filtered_data <- plotting_data %>%
      select(location, date, total_vaccinations_per_hundred, new_deaths_smoothed_per_million, iso_code, positive_rate) %>%
  ...
  output$heatmap_plot <- renderPlot({
    ...
    print(heatmap_plot)
  }, width = 1500, height = 1000, res = 96)  # Adjust width and height as needed
